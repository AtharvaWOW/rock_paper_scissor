...
char getComputerOption() {//generates random choice for the computer from r/p/s

    srand(time(0));//time(0) retrieves current time in seconds
    // Random number
    int num = rand() % 3 + 1;//rand generates a large number between 0 and inf
    //we get a number from 1,2,3
